...
import rhythm.Song;
import rhythm.Section.SwagSection;
import rhythm.Note.EventNote;
import rhythm.JudgmentManager;

#if PE_MOD_COMPATIBILITY
//import objects.sprites.FNFHealthBar; //Later. For now use `import states.PlayState.FNFHealthBar;`
import objects.sprites.HealthIcon;
#end
...
#if discord_rpc
import startup.Discord.DiscordClient;
#end
	...
	*/
	...
	override public function create()
	{
		...
		for (judgeData in judgeManager.judgmentData)
			assetsToLoad.push({path: judgeData.internalName});

		for (i in 0...10)
			assetsToLoad.push({path: 'num$i'});

		for (i in 1...3) // TODO: Be able to add more than 3 miss sounds
			assetsToLoad.push({path: 'missnote$i', type: 'SOUND'});

		assetsToLoad.push({path: 'hitsound', type: 'SOUND'});
		assetsToLoad.push({path: "healthBar"});
		assetsToLoad.push({path: "timeBar"});

		/* 
		if (PauseSubState.songName != null)
			assetsToLoad.push({path: PauseSubState.songName, type: 'MUSIC'});
		else if (ClientPrefs.pauseMusic != 'None')
			assetsToLoad.push({path: Paths.formatToSongPath(ClientPrefs.pauseMusic), type: 'MUSIC'}); 
		assetsToLoad.push({path: "breakfast", type: 'MUSIC'}); 
		*/

		////
		if (ClientPrefs.noteSkin == 'Quants'){
			assetsToLoad.push({path: 'QUANT$arrowSkin'});
			assetsToLoad.push({path: 'QUANT$splashSkin'});
		}else{
			assetsToLoad.push({path: arrowSkin});
			assetsToLoad.push({path: splashSkin});
		}
		...
		if (stageData.preloadStrings != null)
		{
			...
			for (i in stageData.preloadStrings)
				assetsToLoad.push({path: i, library: lib});
		}
		...
		// Paths.getAllStrings();
		Cache.loadWithList(assetsToLoad);
		assetsToLoad = [];
		...
		*/
		...
		for (introSndPath in introSnds){
			if (introSndPath != null)
				assetsToLoad.push({path: introSndPath, type: "SOUND"});
		}
		for (introImgPath in introAlts){
			if (introImgPath != null)
				assetsToLoad.push({path: introImgPath});
		}
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (Std.isOfType(type, Int)) 
					type = states.editors.ChartingState.noteTypeList[type];
				*/
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (Std.isOfType(songNotes[3], Int))
					type = states.editors.ChartingState.noteTypeList[songNotes[3]]; //Backward compatibility + compatibility with Week 7 charts;
				*/
