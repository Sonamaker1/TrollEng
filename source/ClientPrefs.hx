...
class ClientPrefs
{
			...
			"gradeSet" => {
				...
				data: [
					"options" => (function()
					{
						...
						for (key in rhythm.scores.Highscore.grades.keys())
							arr.push(key);
	...
	{
		return ((!states.PlayState.isStoryMode && gameplaySettings.exists(name)) ? gameplaySettings.get(name) : defaultValue);
	}
